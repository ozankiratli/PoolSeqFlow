...
- **Both closed lists are refusals, not filters.** A `param_` or `RG_` column the pipeline did not recognize would be a setting the user has written down, can see in their own file, and that was never applied. The parser also refuses a *bare* `poolSize`/`adapter1` — without that check it would be accepted as design metadata and silently ignored, which is the exact failure the prefixes exist to prevent.
