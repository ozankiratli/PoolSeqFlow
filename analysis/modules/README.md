...
17c. **Degrees of freedom come from `design.units`, never from `design.series`.** A **unit** is one independent biological entity. Two pools are two units unless a column named in `analysis.metadata.design.technicalRep` says they are one material measured twice — two lanes of one cage are one unit, three control pools with nothing to tell them apart are three. **`design.series` is empty for a project with no time axis and `design.units` is not**, so a module that reads units works on a one-off comparison and one that reads series does not. Each unit carries its own `pools`, sorted as `design.pools` is, so index the published columns with those and never re-derive the grouping. `design.conditions` is the level above: units with the biological-replicate columns dropped too, each carrying the `units` it holds.

17d. **An empty `analysis.metadata.design.biologicalRep` is not a claim that there are no replicates.** The frame declares nothing it was not told. If your test stratifies on biological replication, refuse by name when that list is empty rather than treating every unit as a stratum of its own.
