...
def libraryFiles() {
    return ['n_eff.R', 'allele_frequencies.R', 'nei_distance.R', 'add_distance.R',
            'mean_distance.R', 'chunk_ranges.R']
}

// The compiled forms this module offers, published beside the result whether or not the run
// used them.
def compiledFiles() {
    return ['allele_frequencies.cpp', 'nei_distance.cpp']
}
...
process Analyze {
    ...
    compiled = compiledFiles().collect { name -> "${installDir()}/analysis/lib/cpp/${name}" }
